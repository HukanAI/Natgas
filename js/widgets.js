...
export function renderFuturesCurve() {
    ...
    // The overview card can be toggled to seasonality — only take over the
    // visible area when the curve view is actually selected.
    const curveActive = (state.fwCardMode || 'curve') === 'curve';
    if (curveActive) {
...
export function updateEIATracker() {
    ...
    if (predictedChange != null) {
        ...
        surpriseLine = 'Surprise: <span style="color:' + col + ';font-weight:700">' + sign + Math.round(surprise) + ' Bcf</span> <span style="color:' + col + '">' + label + '</span>';
    } else {
