...
// Returns Set of ISO date strings (YYYY-MM-DD) for NYMEX holidays in given year
function nymexHolidays(year) {
    const set = new Set();
    function add(d) { set.add(d.toISOString().slice(0, 10)); }
    function adjustObserved(d) {
    ...
    // Nth weekday of month: nth=1..4, dow=0..6
    ...
    add(adjustObserved(new Date(Date.UTC(year, 0, 1))));                  // New Year's Day
    add(nthWeekday(year, 0, 3, 1));                                       // MLK Day (3rd Mon Jan)
    add(nthWeekday(year, 1, 3, 1));                                       // Presidents Day (3rd Mon Feb)
    ...
    add(adjustObserved(new Date(Date.UTC(year, 5, 19))));                 // Juneteenth (Jun 19)
    add(adjustObserved(new Date(Date.UTC(year, 6, 4))));                  // Independence Day (Jul 4)
    add(nthWeekday(year, 8, 1, 1));                                       // Labor Day (1st Mon Sep)
    add(nthWeekday(year, 10, 4, 4));                                      // Thanksgiving (4th Thu Nov)
    add(adjustObserved(new Date(Date.UTC(year, 11, 25))));                // Christmas (Dec 25)
    return set;
}

// Cache holiday sets per year
...
export function updateRollKpi() {
    ...
    function nymexLastTradingDay(year, deliveryMonth) {
    ...
    // T212 broker roll = 2 business days before NYMEX expiry
    function t212RollDate(year, deliveryMonth) {
        ...
        // Settlement near end of trading day (~14:30 ET ≈ 19:30 UTC for simplicity)
        expiry.setUTCHours(19, 30, 0, 0);
        return expiry;
    }
    ...
    // If today is before this month's roll, target is this month's delivery
    // Otherwise target is next month's delivery
    ...
    // Color: imminent (≤1d) = red, close (≤3d) = yellow, normal = green
    if (card) {
