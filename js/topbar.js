...
function previousDayClose(kind) {
    ...
    const now = new Date();
    const todayYMD = now.getUTCFullYear() + '-' + (now.getUTCMonth()+1) + '-' + now.getUTCDate();
    // Scan from most recent backwards, skip bars that match today's date
    for (let i = bars.length - 1; i >= 0; i--) {
        const d = new Date(bars[i].ts);
        const ymd = d.getUTCFullYear() + '-' + (d.getUTCMonth()+1) + '-' + d.getUTCDate();
        if (ymd !== todayYMD) return bars[i].close;
    }
    // All bars are today (shouldn't happen with 10-day range, but safe)
    return null;
}
...
document.addEventListener('ngf:price:refresh', function(e) {
    ...
    // Use prev-day close from dailyHistory (fallback to event's prev if not yet loaded)
    const prevFromHistory = previousDayClose(isNext ? 'next' : 'front');
    const prev = prevFromHistory != null ? prevFromHistory : e.detail.prev;
    if (last == null) return;
