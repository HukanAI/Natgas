...
async function fetchRaw(interval, rangeStr) {
  ...
  // Resolve ticker: 'front' = NG=F (continuous front month), 'next' = next contract symbol
  let ticker = 'NG=F';
  if (state.taTicker === 'next') {
    const cur = ngfCurrent();
    const nxt = cur ? ngfNext(cur) : null;
    if (nxt && nxt.ticker) ticker = nxt.ticker;
  }
...
export function taRenderTF(tf, candles) {
  ...
  state.taCharts[tf].price = new Chart(
    ...
    {
      type: 'line',
      plugins: [candlePlugin, patPlugin, smcLabelPlugin(() => state.taCharts[tf]._smcLabels || [])],
      data: {
        labels: labels,
        datasets: [
