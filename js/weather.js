...
export function wxRenderMetrics() {
  if (!state.wxS) return;
  const si=getSeasonInfo(), ti=state.wxS.todayIdx;
  ...
  function fillDemBox(valId,vs5yId,avgId,days) {
    const dem=demSum(days),dem5=dem5Sum(days),dev=dem-dem5,pct=dem5>0.5?dev/dem5*100:0;
    const col=dev>0?'#3fb950':dev<0?'#ff7b72':'#e3b341';
    const el=document.getElementById(valId); el.textContent=dem.toFixed(0); el.style.color=col;
    document.getElementById(vs5yId).innerHTML=dem5>0.5
      ?'<span style="color:'+col+'">'+(dev>=0?'+':'')+dev.toFixed(0)+' ('+(dev>=0?'+':'')+pct.toFixed(1)+'%) vs 5y</span>'
      :'<span style="color:var(--text3)">vs 5y N/A</span>';
    const dAvg=dem/days, d5Avg=dem5>0?dem5/days:0, ac=dAvg>d5Avg?'#3fb950':dAvg<d5Avg?'#ff7b72':'#6e7681';
    document.getElementById(avgId).innerHTML='Ø '+dAvg.toFixed(1)+'/d · <span style="color:'+ac+'">5y: '+d5Avg.toFixed(1)+'</span>';
  }
  [4,7,10,13,16].forEach(d=>fillDemBox('wx-dem'+d,'wx-dem'+d+'-vs5y','wx-dem'+d+'-avg',d));

  let hddFwd=0,cddFwd=0,lim16=Math.min(ti+16,state.wxS.hddAll.length);
  for (let k=ti;k<lim16;k++){hddFwd+=state.wxS.hddAll[k]||0;cddFwd+=state.wxS.cddAll[k]||0;}
  const totFwd=hddFwd+cddFwd;
  const dEl=document.getElementById('wx-driver'), dSub=document.getElementById('wx-driver-sub');
  if (totFwd<0.5) { dEl.textContent='—'; dEl.style.color='#8b949e'; dSub.textContent='no demand'; }
  else if (si.name==='Shoulder') {
    const hP=Math.round(hddFwd/totFwd*100), cP=100-hP;
    if (hP>=cP) { dEl.textContent='HDD '+hP+'%'; dEl.style.color='#4493f8'; dSub.textContent='CDD '+cP+'% · shoulder'; }
    else         { dEl.textContent='CDD '+cP+'%';  dEl.style.color='#ff7b72'; dSub.textContent='HDD '+hP+'% · shoulder'; }
