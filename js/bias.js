...
// ── Fair price box helper ─────────────────────────────────────────────────────

function renderFairBox(vid, rid, devBcf, isH) {
  const fp=fairPrice(devBcf), mn=fp-0.5, mx=isH?fp+1.9:fp+0.5;
  const el=document.getElementById(vid); el.textContent='$'+fp.toFixed(3); el.style.color='#e6edf3';
...
export function renderBiasCard() {
  ...
  document.getElementById('b-stor-dev').innerHTML=devBcf!=null
    ?'<span style="color:'+(devBcf>=0?'#ff7b72':'#3fb950')+'">'+sgn(devBcf)+Math.round(devBcf).toLocaleString()+' Bcf ('+sgn(devPct)+devPct.toFixed(1)+'%) vs 5y</span>'
    :'5y avg N/A';
  ...
  function fillF(vid,cid,vsid,dateid,fcst,base,vsLbl){
    ...
    if (b5r!=null) { const dv=lv-b5r,pv=b5r!==0?dv/b5r*100:0; document.getElementById(vsid).innerHTML='<span style="color:'+(dv>=0?'#ff7b72':'#3fb950')+'">'+sgn(dv)+Math.round(dv)+' Bcf ('+sgn(pv)+pv.toFixed(1)+'%) vs 5y</span>'; }
  ...
  if (state.stLastF7?.predictedLevel!=null)  { const b=st5y(state.stStorageData,[state.stLastF7.endDate])[0].avg;  if(b!=null) renderFairBox('b-fp7', 'b-fp7-range', state.stLastF7.predictedLevel-b,isH); }
  if (state.stLastF14?.predictedLevel!=null) { const b=st5y(state.stStorageData,[state.stLastF14.endDate])[0].avg; if(b!=null) renderFairBox('b-fp14','b-fp14-range',state.stLastF14.predictedLevel-b,isH); }
  if (state.stLastF21?.predictedLevel!=null) { const b=st5y(state.stStorageData,[state.stLastF21.endDate])[0].avg; if(b!=null) renderFairBox('b-fp21','b-fp21-range',state.stLastF21.predictedLevel-b,isH); }
