...
function renderMobileStorage() {
    ...
    // Get 5y avg for current date to compute deviation
    const { st5y } = window._st5y || {};
    if (!st5y) {
        if (valEl) { valEl.textContent = Math.round(lat.value).toLocaleString(); valEl.style.color = 'var(--text)'; }
        if (detEl) detEl.textContent = lat.date;
        return;
    }
    const band = st5y([lat.date])[0];
    if (!band?.avg) return;
    const dev = Math.round(lat.value - band.avg);
    const col = dev < 0 ? '#3fb950' : '#ff7b72';
    if (valEl) { valEl.textContent = (dev >= 0 ? '+' : '') + dev.toLocaleString(); valEl.style.color = col; }
    if (detEl) detEl.textContent = Math.round(lat.value).toLocaleString() + ' Bcf · ' + lat.date;
}
