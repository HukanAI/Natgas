...
async function loadDailyHistory() {
  ...
  const [front, next] = await Promise.all([
    fetchDailyHistory('NG=F').catch(e => { dbLog('Daily history front fail: ' + e.message, 'warn'); return null; }),
  ...
  if (front || next) {
    dbLog('Daily history loaded: front=' + (front?.length||0) + ' bars · next=' + (next?.length||0) + ' bars', 'ok');
    // Trigger topbar refresh so it uses new prev close
